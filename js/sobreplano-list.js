...
(() => {
  ...
  const cardHTML = (project) => {
    ...
    const media = project.image
      ...
      : `<span class="img-placeholder__label">IMAGEN DEL PROYECTO PRÓXIMAMENTE</span>`;
    const technologyBadges = [
      project.glb ? '<span class="tech-badge">3D</span>' : "",
      project.usdz ? '<span class="tech-badge">AR</span>' : "",
    ].join("");
    return `
      <article class="project-card reveal is-visible" data-status="${project.status}">
        <a href="${href}" class="project-card__media img-placeholder${project.image ? " has-image" : ""}" aria-label="${project.name}">
          ${media}
          <span class="project-card__badge ${statusClass}">${project.statusLabel}</span>
          <span class="project-card__technology">${technologyBadges}</span>
        </a>
        <div class="project-card__body">
          <p class="project-card__type">Lote + vivienda predeterminada</p>
          ...
          <div class="project-card__meta">
            <div><span>Área del lote</span><strong>${project.lotArea || "Por confirmar"}</strong></div>
            <div><span>Área construida</span><strong>${project.builtArea || "Por confirmar"}</strong></div>
          </div>
          <p class="project-card__model"><span>Vivienda incluida</span><strong>${project.houseModel || "Por confirmar"}</strong></p>
          <p class="project-card__price">${project.priceFrom}</p>
          <div class="project-card__actions">
            <a href="${href}" class="btn btn--outline btn--sm">Ver proyecto</a>
            <a href="${href}#proyecto-incluido" class="btn btn--gold btn--sm">Explorar proyecto</a>
          </div>
        </div>
      </article>`;
  };
